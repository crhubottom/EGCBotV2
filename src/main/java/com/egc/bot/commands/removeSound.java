...
public class removeSound implements ICommand {
    ...
    public void run(SlashCommandInteraction ctx) {
        ...
        boolean deleted = matchedFile.delete();

        if (deleted) {
            ctx.getHook().sendMessage("Deleted sound: " + soundName).queue();
        } else {
            ctx.getHook().sendMessage("Failed to delete sound.").queue();
        }
    }
}
